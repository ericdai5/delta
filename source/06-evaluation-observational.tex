...
\paragraph{Duration.}
Each task lasted up to 30 minutes, after which the facilitator stopped the author. Although the tasks were open-ended with no fixed completion criteria, all ten participants produced a working interactive formula with correct computation and semantics behavior and had time to iterate on its design within this limit. Including tutorials, questionnaires, and transitions, each session lasted approximately 2 hours.

\paragraph{Measurement and Analysis.}
9 of 10 sessions were facilitated by the same researcher; 1 session was facilitated by a second researcher following the same protocol. During the task, participants were told to think-aloud and told the facilitator might interrupt them with questions about their process. We also asked them to reflect on their experience in post-task questionnaires after each task. The questionnaire asked participants to rate usefulness, intuitiveness, and difficulty of using each of Delta's main features on Likert scales and elaborate in free text responses. Because each task introduced different Delta features, the post-task questionnaires asked participants to rate only the features relevant to that task—basic interaction and notation features for T1, and stepping features for T2. After the session, both researchers independently reviewed their respective recordings and compiled timestamped observation logs for each participant, documenting feature usage patterns, error sequences, recovery strategies, and notable think-aloud utterances. One researcher then performed qualitative coding, reviewing all data, identifying common workflows, design successes and gaps. The codes retained were those that were seen as lending insight into how to design future DSLs like Delta.
